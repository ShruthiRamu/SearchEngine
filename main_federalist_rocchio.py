...
def index_corpus(corpus: DocumentCorpus) -> (Index, List[float], List[int], int, float, float):
    ...
    for d in corpus:
        ...
        for t, tftd in term_tftd.items():
            wdt = 1 + ln(tftd)
            # wdt = round(wdt, 2)
            # print(f"Term: {t}, tftd: {tftd}, wdt: {wdt}")
        ...
        for t in vd_d.keys():
            vd_d[t] = vd_d[t] / Ld  # Neal gets Ld from disk index and also gets wdt from the disk postings(maybe
            # do this in the main)
    ...
    # centroid = dict(reduce(lambda x, y: Counter(x) + Counter(y), sorted_vds))
    ...
    return index, centroid, document_weights, document_tokens_length_per_document, byte_size_ds, average_tftds, document_tokens_length_average


def get_vd_disputed(corpus: DocumentCorpus) -> (Index, List[float], List[int], int, float, float):
    print("Calculating V(d) for disputed document.....")
    token_processor = NewTokenProcessor()
    index = PositionalInvertedIndex()
    document_weights = []  # Ld for all documents in corpus
    document_tokens_length_per_document = []  # docLengthd - Number of tokens in a document
    ...
    for d in corpus:
        ...
        vds.append(vd_d)
        # print(f"V(d) for disputed file: {vds}")
        document_weights.append(Ld)
        # docLengthd - update the number of tokens for the document
        document_tokens_length_per_document.append(document_tokens_length_d)
        # update the sum of tokens in all documents
    ...
    # docLengthA - average number of tokens in all documents in the corpus
    document_tokens_length_average = 0

    return index, vds, document_weights, document_tokens_length_per_document, byte_size_ds, average_tftds, document_tokens_length_average
...
if __name__ == "__main__":
    ...
    hamilton_index, hamilton_centroid, hamilton_document_weights, hamilton_document_tokens_length_per_document, hamilton_byte_size_d, hamilton_average_tftd, \
    ...
    jay_index, jay_centroid, jay_document_weights, jay_document_tokens_length_per_document, jay_byte_size_d, jay_average_tftd, \
    ...
    madison_index, madison_centroid, madison_document_weights, madison_document_tokens_length_per_document, madison_byte_size_d, madison_average_tftd, \
    ...
    disputed_index, vds, disputed_document_weights, disputed_document_tokens_length_per_document, disputed_byte_size_d, disputed_average_tftd, \
    disputed_document_tokens_length_average = get_vd_disputed(disputed_corpus)

    distance = 0.
    for key, value in vds[0].items():
        # Get the first value for the term from the centroid
        if not hamilton_centroid.get(key):
            first_value = 0
        else:
            first_value = hamilton_centroid.get(key)
    ...
    print("\n\nDist to HAMILTON for doc paper_49.txt is: ", distance)

    distance = 0.
    for key, value in vds[0].items():
        # Get the first value for the term from the centroid
        if not jay_centroid.get(key):
            first_value = 0
        else:
            first_value = jay_centroid.get(key)
        # Get the second value for the term from the v(d) of disputed document
        second_value = value
        ...
        distance += squared_difference
    distance = sqrt(distance)
    print("\n\nDist to JAY for doc paper_49.txt is: ", distance)

    distance = 0.
    for key, value in vds[0].items():
        # Get the first value for the term from the centroid
        if not madison_centroid.get(key):
            first_value = 0
        else:
            first_value = madison_centroid.get(key)
        # Get the second value for the term from the v(d) of disputed document
        second_value = value
        ...
        distance += squared_difference
    distance = sqrt(distance)
    print("\n\nDist to MADISON for doc paper_49.txt is: ", distance)
